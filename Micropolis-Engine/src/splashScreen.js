...
var acquireNameAndDifficulty = function(e) {
  ...
  // When the form is submitted, we'll be ready to launch the game. Starcade:
  // also bind a direct click handler on the submit button itself as a
  // fallback - some embedded webview environments don't reliably propagate
  // native HTML form-submit semantics the way a desktop browser does, even
  // though a real click on the button still fires. playHandled below guards
  // against both firing for the same real click in environments where
  // submit *does* work normally (a click on a submit button naturally raises
  // both a click and a submit event).
  var playHandled = false;
  var playOnce = function(e) {
    if (playHandled) { e.preventDefault(); return; }
    playHandled = true;
    play.call(this, e);
  };
  $('#playForm').submit(playOnce.bind(this));
  $('#playit').click(playOnce.bind(this));
